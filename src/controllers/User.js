...
module.exports = {
  ...
  create: async function (req, res, next) {
    ...
    if (admin === "" || admin === " " || (admin != true && admin != false)) {
      errors.push({ error: "Admin is invalid" });
    }

    if (validation.validationCpf(cpf) === false) {
      errors.push({ error: "CPF is invalid" });
    }

    if (validation.validationPassword(password) === false) {
      ...
      await UserAdress.create({
        ...
        .then((result) => {
          ...
          const mailOptions = {
            ...
            html:
              '<div style="background-color: #E0FFFF; color: #363636; text-align: center; margin-right: 5%; margin-left:5%"><h1 style=" padding-top: 50px">Olá ' +
              name +
              '! </h1><h3> Este email é para te informar que o seu cadastro no aplicativo rede cegonha foi realizado com sucesso</h3><h3 style=" padding-bottom: 50px"> Acesse sua conta com o usuário  ' +
              username +
              "</h3></div>",
          };
          transporter.sendMail(mailOptions, (err, info) => {
            ...
            return res
              .status(200)
              .json({ message: "A new password was sent to email " + email });
          });
